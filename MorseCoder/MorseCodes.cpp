...
typedef struct {      // A struct is like a simple class with just public fields (no methods)
  ...
  const char *codeString;   // The Encoded value to substitute for the symbol
} Code;  
...
const char* morseEncode(char symbol) {
  ...
  for(unsigned i=26+10;i<MorseCodeTableSize;i++) {
...
char morseDecode(const char* encoded) {
  ...
  for(unsigned i=0;i<MorseCodeTableSize;i++) {
